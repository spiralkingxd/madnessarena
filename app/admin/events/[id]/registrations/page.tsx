...
export default async function AdminEventRegistrationsPage({ params }: Props) {
  ...
  const { event, registrations, availableTeams } = await getEventRegistrations(id);
      ...
      <header className="rounded-2xl border border-white/10 bg-slate-950/60 p-6">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href={`/admin/events/${id}/registrations/export`} className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-white/10">
