...
export function JoinRequestButton({
  ...
  async function onCancelRequest() {
    ...
    const confirmed = window.confirm("Deseja cancelar sua solicitação de entrada nesta equipe?");
    if (!confirmed) return;
      ...
      {state === "pending" ? (
        <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          <button
            ...
            aria-label="Solicitação pendente"
          ...
          <button
            ...
            aria-label="Cancelar solicitação pendente"
