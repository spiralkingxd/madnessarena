...
export function ProfileTeamsSection({ userId, teams, teamsError }: Props) {
  ...
  return (
    <section id="teams" className="rounded-3xl border border-white/10 bg-slate-900/60 p-6 shadow-xl shadow-black/20">
      ...
      {orderedTeams.length > 0 ? (
          ...
          {orderedTeams.map((team) => {
            ...
            return (
              <Link
                key={team.id}
                href={`/teams/${team.id}`}
                className="group rounded-2xl border border-white/10 bg-white/4 p-4 transition hover:border-amber-400/30 hover:bg-amber-400/6"
              >
                <div className="flex items-start gap-3">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white/5">
                    {team.logo_url ? (
                      <Image
                        src={team.logo_url}
                        alt={team.name}
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-white/2 px-5 py-10 text-center">
